  ...
  function startGettingLocation()
  {
    try
    {
      HighFrequency = document.getElementById("highfrequency").checked;
      if(HighFrequency) {logError("High Frequency Turned on");}
      else {
        logError("High Frequency off");
      }
    if(!startedLocation)
    {
      watchID = navigator.geolocation.watchPosition(onSuccess, onError, {timeout:20000, enableHighAccuracy:HighFrequency});
      document.getElementById("start-button").innerHTML = "Stop getting Speed";
      startedLocation = true;
    }
    else
    {
      navigator.geolocation.clearWatch(watchID);
      Point1 = {Latitude:-1,Longitude:-1,TimeStamp:0};
      document.getElementById("start-button").innerHTML = "Start getting Speed";
      startedLocation = false;
    }
  }
  catch(e)
  {
    alert("An error occurred in startGettingLocation: " + e.message );
  }
}
...
function onSuccess(position)
{
  try
  {
    //This is the first point we need to initialize
    if(Point1.Latitude === -1 && Point1.Longitude === -1)
    {
      Point1.Latitude = position.coords.latitude;
      Point1.Longitude = position.coords.longitude;
      Point1.TimeStamp = position.timestamp;
      return;
    }
    ...
    var distanceinM = (getDistanceFromLatLonInKm(Point1.Latitude,Point1.Longitude,Point2.Latitude,Point2.Longitude) * 1000);
...
{
  document.getElementById("ErrorLog").innerHTML = str + "\n";
}
  // onError Callback receives a PositionError object
      function onError(error) {
          logError('code: '    + error.code + ' message: ' + error.message + '\n');
      }
